...
func _ready():
	...
	#name_filter.text_changed.connect(_on_filter_changed)
...
func _on_filter_changed(_index: int):
	...
	#name_query = name_filter.text.to_lower()
	_apply_filters()
...
func _apply_filters():
	for child in grid_container.cards_container.get_children():
		...
		#var matches_evolved = (selected_evolved == "All" or child.metadata.evolved == selected_evolved)
		#var matches_name = (name_query.is_empty() or child.metadata.name.to_lower().contains(name_query))
		var matches_trait = (trait_query.is_empty() or child.metadata.card_trait.to_lower().contains(trait_query))
		child.visible = matches_type and matches_class and matches_universe and matches_rarity and matches_trait
